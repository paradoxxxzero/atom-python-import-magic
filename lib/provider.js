...
export default {
  ...
  getSuggestsFromCompletions(completions, prefix, fullPrefix) {
    ...
    return completions[1]
      ...
      .map(comp => ({
        text: comp,
        displayText:
          fullPrefix && comp.startsWith(fullPrefix)
            ? comp.slice(fullPrefix.length - prefix.length)
            : comp,
        replacementPrefix: fullPrefix || prefix,
        type: fullPrefix ? 's' : 'w',
        rightLabel: fullPrefix
          ? comp.startsWith(fullPrefix) ? '+' : '*'
          : null,
  ...
  getSuggestions({ editor, bufferPosition, prefix, activatedManually }) {
    return new Promise(resolve => {
      if (
        !activatedManually ||
        !atom.config.get('python-import-magic.autocompleteImports')
      ) {
        return resolve([])
      }
      ...
      return call(
        ...
        out =>
          resolve(
            Array.from(out.imports || []).map(imp => ({
              text: imp,
              type: 'import',
            }))
          )
  ...
  onDidInsertSuggestion({ editor, suggestion }) {
    ...
    call(
      ...
      editor,
      out => editor.getBuffer().setTextViaDiff(out.file)
    )
  },
}
